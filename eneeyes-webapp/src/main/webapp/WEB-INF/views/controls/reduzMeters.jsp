...
<span data-ng-if="item.artefact!='GAS'">{{item.artefact}}</span>

<span data-ng-if="item.artefact=='GAS' && item.deviceType=='DETECTOR'">Gás: {{item.gasName}}</span>
<span data-ng-if="!item.artefact">{{item.gasDto.name}}</span>
