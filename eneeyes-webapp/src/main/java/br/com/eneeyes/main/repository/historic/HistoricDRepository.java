...
import br.com.eneeyes.main.model.historic.HistoricD;
import br.com.eneeyes.main.model.historic.IHistoric;

public interface HistoricDRepository extends JpaRepository<HistoricD, Long> {
		
//	@Query("select h from HistoricD h where h.companyDetectorId = ?1 and h.sensorId = ?2 and h.lastUpdate between ?3 and ?4")
...
//	public Page<IHistoric> findByCompanyDetectorIdAndSensorIdAndLastUpdateBetweenPaginated(Long companyDetectorId, Long sensorId, Date in, Date out, Pageable pageable);
	...
	@Query("select h from HistoricD h where h.companyDetectorId = ?1 and h.lastUpdate between ?2 and ?3")
