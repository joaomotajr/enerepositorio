...
import br.com.eneeyes.main.model.historic.day.HistoricCDayView;

public interface HistoricCDayViewRepository extends JpaRepository<HistoricCDayView, Long> {
	
	@Query("select h from HistoricCDayView h where h.companyDeviceId = ?1 and h.lastUpdate between ?2 and ?3")
	public List<IHistoricGroup> findByCompanyDeviceIdAndLastUpdateBetween(Long companyDeviceId, Date in, Date out);

	@Query("select h from HistoricCDayView h where h.companyDeviceId = ?1 and h.lastUpdate between ?2 and ?2 ")
