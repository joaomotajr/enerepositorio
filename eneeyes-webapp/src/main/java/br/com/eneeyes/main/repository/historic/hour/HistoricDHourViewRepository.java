...
import br.com.eneeyes.main.model.historic.hour.HistoricDHourView;

public interface HistoricDHourViewRepository extends JpaRepository<HistoricDHourView, Long> {
	
	@Query("select h from HistoricDHourView h where h.companyDeviceId = ?1 and h.lastUpdate between ?2 and ?3")
	public List<IHistoricGroup> findByCompanyDeviceIdAndLastUpdateBetween(Long companyDeviceId, Date in, Date out);

	@Query("select h from HistoricDHourView h where h.companyDeviceId = ?1 and h.lastUpdate between ?2 and ?2 ")
	...
	@Query("select h from HistoricDHourView h where h.companyDeviceId in ?1 and h.lastUpdate between ?2 and ?3")
