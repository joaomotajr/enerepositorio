...
SELECT 
    ...
    FROM
        `historic`
    GROUP BY `historic`.`COMPANY_DETECTOR_ID` , DATE_FORMAT(`historic`.`LAST_UPDATE`, '%d/%m/%Y')
...
SELECT 
    ...
    FROM
        historic
    GROUP BY COMPANY_DETECTOR_ID, TO_char(historic.LAST_UPDATE, 'YYYY-MM-DD 00:00:00')
