...
public interface HistoricViewHourRepository extends JpaRepository<HistoricViewHour, Long> {
	...
	@Query("select h from HistoricViewHour h where h.companyDeviceId = ?1 and h.lastUpdateHour between ?2 and ?3")
	...
	@Query("select h from HistoricViewHour h where h.companyDeviceId in ?1 and h.lastUpdateHour between ?2 and ?3")
	public Page<IHistoricHourGroup> findByCompanyDeviceInAndLastUpdateBetweenPaginated(List<Long> companyDevice,
